...
instant a trader could have acted on it. Joins run backward with an explicit tolerance,
and the row records how old its oldest input is.
...
def to_utc(naive: pd.Series, tz: str) -> pd.Series:
    return naive.dt.tz_localize(tz, nonexistent="shift_forward", ambiguous="raise").dt.tz_convert(
        "UTC"
    )


def read_prices(path: Path) -> pd.DataFrame:
    """One headerless date,value file. Read as text then coerced, so a new sentinel
    fails the build rather than arriving as a silent NaN."""
...
def expiry_cutoff(product: str, delivery_month: pd.Timestamp) -> pd.Timestamp:
...
def load_futures(folder: Path, products: tuple[str, ...]) -> pd.DataFrame:
    frames = []
    for path in sorted(folder.glob("*.csv")):
        ...
        delivery_month = pd.Timestamp(year, MONTH_CODES[code], 1)
        prices = read_prices(path)

        # A settle printing after the contract can plausibly trade means the file holds
        # some other series, which is exactly what a fetcher falling back to the
        # continuous front month produces. Refuse it rather than net back a wrong month.
        ...
        tz, offset = SETTLE_ANCHORS[product]
        frames.append(
            prices.assign(
                product=product,
                contract=path.stem,
...
def load_lng(folder: Path = LNG_DIR) -> pd.DataFrame:
...
def load_freight(folder: Path = FREIGHT_DIR) -> pd.DataFrame:
...
def load_fx(folder: Path = FX_DIR) -> pd.DataFrame:
    ...
    tz, offset = FX_AVAILABLE
    df["available_timestamp_utc"] = to_utc(df["trade_date"] + offset, tz)
    return df[["trade_date", "available_timestamp_utc", "eurusd"]].sort_values(
...
def loading_month(trade_date: pd.Timestamp) -> pd.Timestamp:
    """A spot cargo loads on the next program, so the month after the trade month."""
    year, month = (
        (trade_date.year + 1, 1) if trade_date.month == 12 else (trade_date.year, trade_date.month + 1)
    )
    return pd.Timestamp(year, month, 1)


def delivery_month(loading: pd.Timestamp, laden_days: int) -> pd.Timestamp:
    """Where the cargo lands: mid-month sailing plus the voyage. Rotterdam at 10 days
    stays in the loading month, Tokyo at 20 days via Panama lands in the next one."""
    arrival = loading + pd.Timedelta(days=LOAD_DAY - 1 + laden_days)
    return pd.Timestamp(arrival.year, arrival.month, 1)


def _join_settles(left: pd.DataFrame, store: pd.DataFrame, on: str, prefix: str) -> pd.DataFrame:
...
def _reject_lookahead(df: pd.DataFrame, column: str, label: str) -> None:
...
def trade_dates(lng: pd.DataFrame) -> pd.DataFrame:
...
def align(spec, grid: pd.DataFrame, lng, freight, fx) -> pd.DataFrame:
    """Everything a trader could see on a date for one route, and how old it is."""
    dest_product, freight_product, laden_days, _canal, _regas = spec

    df = grid.copy()
    loading = df["trade_date"].map(loading_month)
    delivery = loading.map(lambda month: delivery_month(month, laden_days))
    df["dest_contract"] = delivery.map(lambda m: f"{dest_product}{CODE_BY_MONTH[m.month]}{m.year}")
    df["freight_contract"] = loading.map(lambda m: f"{freight_product}{CODE_BY_MONTH[m.month]}{m.year}")

    df = _join_settles(df, lng[lng["product"] == dest_product], "dest_contract", "dest")
    df = _join_settles(df, freight[freight["product"] == freight_product], "freight_contract", "freight")
    _reject_lookahead(df, "dest_settle_utc", "destination settle")
    _reject_lookahead(df, "freight_settle_utc", "freight settle")

    needed = ["dest_settle", "freight_settle"]
    if dest_product == "TTF":
        # A bar labelled D is only available at D+1 17:00 New York, which is after this
        # row's anchor, so only labels up to D-1 are provably known.
        df = pd.merge_asof(
            df,
            fx.assign(fx_price_date=fx["trade_date"]).sort_values("trade_date"),
            on="trade_date",
            direction="backward",
            tolerance=TOLERANCE,
            allow_exact_matches=False,
        )
        _reject_lookahead(df, "available_timestamp_utc", "FX bar")
        needed.append("eurusd")
    else:
        df["eurusd"] = float("nan")  # JKM is quoted in USD and needs no conversion

    df = df.dropna(subset=needed)
    ages = [(df["trade_date"] - df[c]).dt.days for c in df if c.endswith("_price_date")]
    return df.assign(max_staleness_days=pd.concat(ages, axis=1).max(axis=1).astype("int64"))


def build_panel(lng: pd.DataFrame, freight: pd.DataFrame, fx: pd.DataFrame) -> pd.DataFrame:
    """One row per (route, trade_date), on the days the LNG market traded."""
    grid = trade_dates(lng)
    panel = pd.concat(
        [netback.price(name, spec, align(spec, grid, lng, freight, fx)) for name, spec in ROUTES.items()],
        ignore_index=True,
    )
...
def main() -> None:
    ...
    with duckdb.connect(str(DB_PATH)) as con:
        ...
        for table, frame in [
            ("lng_futures_daily", lng),
            ("freight_futures_daily", freight),
            ("fx_daily", fx),
            ("netback_panel", panel),
        ]:
